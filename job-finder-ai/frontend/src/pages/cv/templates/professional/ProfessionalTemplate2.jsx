// src/pages/cv/templates/professional/ProfessionalTemplate2.jsx
import React from 'react';
import EditableField from '../../components/EditableField';

const ProfessionalTemplate2 = ({ data, onUpdate }) => {
  ...
  const addVolunteer = () => {
    const newVolunteers = [...(data.volunteers || []), { title: '', organization: '', year: '' }];
    onUpdate({ ...data, volunteers: newVolunteers });
  };

  const updateVolunteer = (index, field, value) => {
    const newVolunteers = [...(data.volunteers || [])];
    if (!newVolunteers[index]) newVolunteers[index] = {};
    newVolunteers[index][field] = value;
    onUpdate({ ...data, volunteers: newVolunteers });
  };

  const removeVolunteer = (index) => {
    const newVolunteers = (data.volunteers || []).filter((_, i) => i !== index);
    onUpdate({ ...data, volunteers: newVolunteers });
  };
      ...
      <div className="p-8 md:p-10">
        ...
        <div className="mb-6 border-b border-gray-200 pb-6">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-1">
            <EditableField
              ...
              placeholder="Tiến sĩ Phan Lan Anh"
              className="font-bold"
            />
          </h1>
          <p className="text-lg text-gray-500 mb-3">
            <EditableField
              ...
              placeholder="Lão khoa"
            />
          </p>
          <div className="flex flex-wrap gap-4 text-sm text-gray-500">
            <EditableField value={data.personalInfo?.linkedin} onSave={(v) => updateField('personalInfo.linkedin', v)} placeholder="LinkedIn.com/in/name" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
                  ...
                  <div key={idx} className="group relative">
                    ...
                    <div className="flex justify-between items-baseline">
                      <h3 className="font-semibold text-gray-800">
                        <EditableField value={exp.position} onSave={(v) => updateExperience(idx, 'position', v)} placeholder="Bác sĩ" />
                      </h3>
                    ...
                    <p className="text-sm text-gray-500 mb-1">
                      <EditableField value={exp.company} onSave={(v) => updateExperience(idx, 'company', v)} placeholder="Bệnh viện An Ninh" />
                    </p>
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-400">KỸ NĂNG LIÊN QUAN</h2>
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                {(data.educations || []).map((edu, idx) => (
                  <div key={idx} className="group relative">
                    ...
                    <h3 className="font-semibold text-gray-800">
                      <EditableField value={edu.school} onSave={(v) => updateEducation(idx, 'school', v)} placeholder="Đại học Trí Đức" />
                    </h3>
                    <p className="text-sm text-gray-500">
                      <EditableField value={edu.degree && edu.fieldOfStudy ? `${edu.degree} - ${edu.fieldOfStudy}` : ''} onSave={(v) => updateEducation(idx, 'degree', v)} placeholder="Bằng Tiến sĩ Y khoa" />
                    </p>
                    <p className="text-xs text-gray-400">
                      <EditableField value={edu.graduationYear} onSave={(v) => updateEducation(idx, 'graduationYear', v)} placeholder="Niên khóa 2021" />
                    </p>
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-400">CÔNG TÁC TÌNH NGUYỆN</h2>
                <button onClick={addVolunteer} className="text-xs text-blue-600">+</button>
              </div>
              <div className="space-y-3">
                {(data.volunteers || []).map((vol, idx) => (
                  <div key={idx} className="group relative">
                    {idx > 0 && (
                      <button onClick={() => removeVolunteer(idx)} className="absolute -top-2 -right-2 text-red-400 hover:text-red-600">✕</button>
                    )}
                    <h3 className="font-semibold text-gray-800 text-sm">
                      <EditableField value={vol.title} onSave={(v) => updateVolunteer(idx, 'title', v)} placeholder="Vai trò" />
                    </h3>
                    <p className="text-xs text-gray-500">
                      <EditableField value={vol.organization} onSave={(v) => updateVolunteer(idx, 'organization', v)} placeholder="Tổ chức" />
                    </p>
                    <p className="text-xs text-gray-400">
                      <EditableField value={vol.year} onSave={(v) => updateVolunteer(idx, 'year', v)} placeholder="Thời gian" />
                    </p>
                  </div>
                ))}
              </div>
