// src/pages/cv/templates/professional/ProfessionalTemplate3.jsx
import React from 'react';
import EditableField from '../../components/EditableField';

const ProfessionalTemplate3 = ({ data, onUpdate }) => {
              ...
              {(data.educations || []).map((edu, idx) => (
                <div key={idx} className="group relative">
                  ...
                  <h3 className="font-semibold text-gray-800">
                    <EditableField value={edu.degree} onSave={(v) => updateEducation(idx, 'degree', v)} placeholder="Nghệ thuật đa phương tiện" />
                  </h3>
                  <p className="text-sm text-gray-500">
                    <EditableField value={edu.school} onSave={(v) => updateEducation(idx, 'school', v)} placeholder="ĐH Mỹ thuật Công nghiệp" />
                  </p>
                  <EditableField value={edu.description} onSave={(v) => updateEducation(idx, 'description', v)} placeholder="Mô tả..." multiline className="text-gray-500 text-sm" />
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-4">
              {(data.experiences || []).map((exp, idx) => (
                <div key={idx} className="group relative">
                  ...
                  <h3 className="font-semibold text-gray-800">
                    <EditableField value={exp.position} onSave={(v) => updateExperience(idx, 'position', v)} placeholder="Thực tập Thiết kế đồ họa" />
                  </h3>
                  <p className="text-sm text-gray-500">
                    <EditableField value={exp.company} onSave={(v) => updateExperience(idx, 'company', v)} placeholder="Công ty Thiết kế ABC" />
                  </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {(data.skills || []).map((skill, idx) => (
              <div key={idx} className="group relative">
                ...
                <h3 className="font-semibold text-gray-800">
                  <EditableField value={skill.name} onSave={(v) => updateSkill(idx, 'name', v)} placeholder="Tên kỹ năng" />
                </h3>
                <p className="text-sm text-gray-500">
                  <EditableField value={skill.level} onSave={(v) => updateSkill(idx, 'level', v)} placeholder="Mô tả" />
                </p>
              </div>
            ))}
          </div>
