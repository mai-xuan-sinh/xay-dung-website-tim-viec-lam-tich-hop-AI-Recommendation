// src/pages/cv/templates/modern/ModernTemplate2.jsx
import React from 'react';
import EditableField from '../../components/EditableField';

const ModernTemplate2 = ({ data, onUpdate }) => {
      ...
      <div className="p-8 md:p-10">
        ...
        <div className="mb-6">
          ...
          <EditableField
            ...
            placeholder="Tôi là Giám đốc Tiếp thị có kinh nghiệm trong ngành Quảng cáo và Tiếp thị kỹ thuật số..."
            multiline
            className="text-gray-600 leading-relaxed"
          />
        </div>

        {/* Kỹ năng chuyên môn + Thông tin liên hệ - 2 cột */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
          <div>
            <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-400 mb-3">KỸ NĂNG CHUYÊN MÔN</h2>
          ...
          <div>
            ...
            <div className="space-y-1 text-sm text-gray-600">
              <div><EditableField value={data.personalInfo?.phone} onSave={(v) => updateField('personalInfo.phone', v)} placeholder="Điện thoại: 0912 3456 7890" /></div>
              ...
              <div><EditableField value={data.personalInfo?.linkedin} onSave={(v) => updateField('personalInfo.linkedin', v)} placeholder="LinkedIn: @trangwebhay" /></div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {(data.educations || []).map((edu, idx) => (
              <div key={idx} className="group relative">
                {idx > 0 && (
                  <button onClick={() => removeEducation(idx)} className="absolute -top-2 -right-2 text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100">✕</button>
                )}
